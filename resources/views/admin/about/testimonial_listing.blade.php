		...
		<div class="container-fluid">
			<div class="inner-body">
				...
				<div class="page-header">
					<div>
						<h2 class="main-content-title tx-24 mg-b-5"> Testimonials </h2>
						<ol class="breadcrumb">
							<li class="breadcrumb-item"><a href="#">Home</a></li>
							<li class="breadcrumb-item active" aria-current="page">Testimonials Pages</li>
						</ol>
					</div>
					<div class="d-flex">
						<div class="justify-content-center">
							<a href="/admin/about/testimonial-add" class="btn btn-white btn-icon-text my-2 mr-2"><i class="fe fe-folder-plus mr-2"></i> Add New Testimonial </a>
						</div>
					</div>
				</div>
				<!-- End Page Header -->
										...
										<table cellpadding="0" cellspacing="0" border="0" class="table table-bordered mg-b-0" id="data-table">
											<thead>
												<tr> 
													...
													<th class="sort" style="width:80px"> Created At </th>
													<th class="sort">Designation & <br /> Company Name </th>  
													<th class="sort">Name</th>  
													<th class="sort" style="width:60px">Thumbnail</th>  
													<th class="sort">Video</th>   
													<th class="sort">Description</th>   
													<th class="text-center sort" style="width:50px"> Status </th>
													<th> Actions </th>
												</tr> 
											</thead>
										{{-- <tbody id="changeTableDate"> --}}
											<tbody>
												...
												<tr class="tableRow" data-id="{{ $item->id }}">
													...
													<td> {{$item->occupation}} <br /> {{$item->designation_company}} </td>
													<td> {{$item->name}} </td>
													<td> <a href="{{$item->thumnail}}" target="_blank"> <img src="{{$item->thumnail}}" class="img-fluid" /> </a> </td> 
													<td> 
														@if($item->video_url != null) 
															<a href="{{$item->video_url}}" target="_blank"> Video </a> 
														@else 
															<span> NA </span> 
														@endif
													</td>
													<td style="white-space: normal !important; word-wrap: break-word;">
														<div class="text-box" data-maxlength="60"> <p class="mb-0"> {{$item->description}} </p> </div>
													</td>                                                    
													<td class="text-center">
														@if($item->status == 0) 
														<a  href="/admin/about/testimonial-status/{{base64_encode($item->status)}}/{{base64_encode($item->id)}}" class="btn btn-danger status_inactive" title="Change Status"><i class='fa fa-times'></i></a> </td>
														@else 
														<a  href="/admin/about/testimonial-status/{{base64_encode($item->status)}}/{{base64_encode($item->id)}}" class="btn btn-primary status-active" title="Change Status"><i class='fa fa-check'></i></a> </td>
														@endif
													</td>						
													<td class="actions">
														<div class="btn-group">
															<a href="/admin/about/testimonial-edit/{{base64_encode($item->id)}}" class="btn ripple btn-info"> Edit  </a>
															<div class="dropdown">
																...
																<div class="dropdown-menu" aria-labelledby="dropdownMenu2"> 
																	{{-- <a href="#"> View </a> --}}
																	<a href="/admin/about/testimonial-delete/{{base64_encode($item->id)}}" onclick="return confirm('Are you sure?')"> Delete </a> 
																</div>
															</div>
														</div> 
													</td>
