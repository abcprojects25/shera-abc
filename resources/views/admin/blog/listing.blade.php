...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/css/toastr.min.css">
    ...
    @else 
    <a href="javascript:void(0)" onclick="changeStatus('/admin/blog/bannerStatus/{{base64_encode($item->is_banner)}}/{{base64_encode($item->id)}}', 'banner')" class="btn btn-primary status-active" title="Change Status"><i class='fa fa-check'></i></a>
    @endif
</td>
<td class="text-center">
    @if($item->is_popular == 0) 
    <a href="javascript:void(0)" onclick="changeStatus('/admin/blog/popularStatus/{{base64_encode($item->is_popular)}}/{{base64_encode($item->id)}}', 'popular')" class="btn btn-danger status_inactive" title="Change Status"><i class='fa fa-times'></i></a>
    @else 
    <a href="javascript:void(0)" onclick="changeStatus('/admin/blog/popularStatus/{{base64_encode($item->is_popular)}}/{{base64_encode($item->id)}}', 'popular')" class="btn btn-primary status-active" title="Change Status"><i class='fa fa-check'></i></a>
    @endif
</td>
<td class="text-center">
    @if($item->status == 0) 
    <a href="javascript:void(0)" onclick="changeStatus('/admin/blog/status/{{base64_encode($item->status)}}/{{base64_encode($item->id)}}', 'status')" class="btn btn-danger status_inactive" title="Change Status"><i class='fa fa-times'></i></a>
    @else 
    <a href="javascript:void(0)" onclick="changeStatus('/admin/blog/status/{{base64_encode($item->status)}}/{{base64_encode($item->id)}}', 'status')" class="btn btn-primary status-active" title="Change Status"><i class='fa fa-check'></i></a>
    @endif
</td>						
													<td class="actions">
														<div class="btn-group">
  ...
  <script>
function changeStatus(url, type) {
    Swal.fire({
        title: 'Are you sure?',
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<ul id="changeTableDate">
					@foreach ($Blogs as $k=>$item)
					<li class="p-2 tableRow" width="20px" data-id="{{ $item->id }}"><span class="m-2">{{$item->blog_order}} </span><img src="{{($item->thumb_image != null) ? $item->thumb_image : '<img src="/admin/img/no_img_xl.jpg" />'}}" height="50px" width="70px" /> {{$item->blog_title}}</li>
					@endforeach
				</ul>
				<br/>
