		...
		<div class="container-fluid">
			<div class="inner-body">
				...
				<div class="page-header">
					<div>
						<h2 class="main-content-title tx-24 mg-b-5"> Product Sub Categories </h2>
						<ol class="breadcrumb">
								...
								<div class="table-responsive">
									<table class="table table-bordered mg-b-0">
										...
										<tr> 
											...
											@if($item->status == 0)
												<td class="text-center"><a href="product-subcategories-status/{{base64_encode($item->status)}}/{{base64_encode($item->id)}}" class="btn btn-danger status_inactive" title="Change Status"><i class='fa fa-times'></i></a> </td>
											@else 
												<td class="text-center"><a href="product-subcategories-status/{{base64_encode($item->status)}}/{{base64_encode($item->id)}}" class="btn btn-primary status-active" title="Change Status"><i class='fa fa-check'></i></a> </td>
											@endif			
											<td class="actions">
												<a href="#" class="btn ripple btn-info"  onclick="editDetails('{{$item->id}}','{{$item->name}}','{{$item->status}}','{{$item->category_id}}')" data-toggle="modal" data-target="#categorieseditmodal"> Edit  </a> 
												<a onclick="return confirm('Are you sure?')" href="product-subcategories-delete/{{base64_encode($item->id)}}" class="btn ripple btn-danger"> Delete </a>
											</td>
										</tr>
										@endforeach
										{{ $Subcategories->links() }}
...
<div class="modal fade" id="categorieseditmodal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-md modal-dialog-centered">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form method="POST" class="row justify-content-md-center" action="/admin/product/subcategory-edit" enctype="multipart/form-data" >	
					<div class="col-xl-12 col-lg-12 col-md-12">
					@csrf			
