...
class AdminAuthController extends Controller
{
    ...
    {
        $Careers = careers::select('id','name','resume','resume_url','created_at')->orderby('id','desc')->take(5)->get();
        ...
        $ProjectsCount = Products::count();
        $SubscribesCount = Subscribes::count();
        $today = Carbon::now()->format('Y-m-d');
        $TodayEnquiries = Enquiry::whereDate("created_at", $today)->count();
        return view('admin.dashboard',compact('Careers','Contacts','EnquiriesCount','careersCount','ProjectsCount','SubscribesCount','TodayEnquiries'));
    }
